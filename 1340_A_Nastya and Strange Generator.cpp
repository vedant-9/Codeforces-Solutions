...
{
    int n; cin>>n; 
    vector<pair<int,int>> a(n); rep(i,0,n) { cin>>a[i].first; a[i].second=i; }
    sort(all(a));
    vector<int> vis(n+1,0); vis[n]=1; 
    int ind = a[0].second+1; vis[ind-1]=1;
    rep(i,1,n) {
        if(a[i].second==ind) { vis[ind]=1; ind++; continue; }
        if(!vis[ind]) {
            cout<<"No\n"; return;
        }
        ind = a[i].second; vis[ind]=1; ind++;
    }
    cout<<"Yes\n";
}

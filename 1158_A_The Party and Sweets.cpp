...
void solve()
{
    ...
    // every col >= every row MUST else -1
    if(g[0]<b[n-1])
    {
        cout<<-1<<endl;
        return;
    ...
    // replicate row value over all grid

    // replace whole row which has max row value with col value equal
    if(g[0]==b[n-1])
        ans = sum1*m + sum2 - b[n-1]*m;
    // replace whole row except 1 element for col max and replace that in 2nd max row
    else
        ans = sum1*m + sum2 - b[n-1]*(m-1) - b[n-2];
    cout<<ans<<endl;
}

...
void solve()
{
    int n,w,x,t; cin>>n>>w; t=w; vector<int> ans;
    vector<pair<int,int>> a(n); rep(i,0,n) {cin>>x; a[i]={x,i};} sort(allr(a));
    rep(i,0,n) {
        if(a[i].first<=t) {
            t-=a[i].first;
    ...
    if(w-t<(w+1)/2) cout<<-1<<endl;
    else { cout<<ans.size()<<endl; out(ans);}
}

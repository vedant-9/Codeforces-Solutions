...
void solve()
{
    // 1. Fill w with all 1's 
    // 2. If s[i]=='0' then do 0 in w for i-x and i+x for valid posn
    // 3. Check for 1's in s[i] if they are valid at least i-x or i+x has '1'

    int x,n,f; string s; cin>>s>>x; n=s.length();
    string w = string(n,'1');
    rep(i,0,n) {
        if(s[i]=='0') {
            if(i+x<n) w[i+x]='0';

...
void solve()
{
    int n,x,mn=INT_MAX,sum=0; cin>>n;
    map<int,int> m; rep(i,0,n) { cin>>x; m[x]++; }
    map<int,int> a; for(auto it:m) a[it.second]++; 
    for(auto it:a) {
        int sum = 0;
        for(auto itd:a) {
            if(it.first!=itd.first) {
                if(itd.first>it.first)
                    sum += itd.second*(itd.first-it.first);
                else
                    sum += itd.second*itd.first;
            }
        }
        mn = min(mn,sum);
    }

...
void solve()
{
    string s,l; cin>>s; int n=s.length(); l=s.substr(n-2);
    int ans=n,i=n-1;
    while(i>=0 and s[i]!='5') i--; i--;
    while(i>=0 and s[i]!='2') i--;
    if(i>=0) ans=min(ans,n-i-2);
    i=n-1;
    while(i>=0 and s[i]!='5') i--; i--;
    while(i>=0 and s[i]!='7') i--;
    if(i>=0) ans=min(ans,n-i-2);
    i=n-1;
    while(i>=0 and s[i]!='0') i--; i--;
    while(i>=0 and s[i]!='5') i--;
    if(i>=0) ans=min(ans,n-i-2);
    i=n-1;
    while(i>=0 and s[i]!='0') i--; i--;

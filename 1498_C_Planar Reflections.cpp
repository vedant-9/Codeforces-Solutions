...
int dp[1005][1005];
ll solver(int front, int back, int decay) {
    if(front==0 || decay==1) return dp[front][decay]=1;
    if(dp[front][decay]!=-1) return dp[front][decay];
    return dp[front][decay]=(solver(front-1,back+1,decay)+solver(back,front,decay-1))%M;
}
